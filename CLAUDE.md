...
After a PR is created, poll the PR status using `gh pr checks --watch` to continuously monitor validation and check results. Once all checks pass, ask the user if the PR can be merged. If the user confirms, merge the PR using `gh pr merge`. After merging, switch back to main and run `git pull` to fetch the merged changes.
